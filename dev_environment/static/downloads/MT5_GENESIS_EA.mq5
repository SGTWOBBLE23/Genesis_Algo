...
void ProcessTradeSignal(int signalId, string symbol, string direction, double entry, double stopLoss, double takeProfit, double lotSize, bool force_execution=false)
{
    ...
    if(isAnticipated) {
        ...
        Print("Placing pending order: ", EnumToString(pendingOrderType), " for ", symbol, " at ", DoubleToString(entry, Digits(symbol)),
              " with SL: ", DoubleToString(stopLoss, Digits(symbol)), " TP: ", DoubleToString(takeProfit, Digits(symbol)),
        ...
        if(success) {
            ...
            Print("Market order executed successfully. Ticket: ", ticket, " Price: ", DoubleToString(price, Digits(symbol)));
            if(SendReports) {
                     ...
                     double entryPrice, double stopLoss, double takeProfit, string status, string message)
{
    ...
    JsonToSend["entry_price"] = DoubleToString(entryPrice, Digits(symbol));
    JsonToSend["stop_loss"] = DoubleToString(stopLoss, Digits(symbol));
    JsonToSend["take_profit"] = DoubleToString(takeProfit, Digits(symbol));
...
bool IsMarketOpenForSymbol(string symbol, bool force_execution=false)
{
    ...
    if(SymbolInfoSessionQuote(symbol, MONDAY, 0, _)) {
        ...
        if(SymbolInfoSessionQuote(symbol, day_of_week, 0, from, to)) {
            if(now >= from && now <= to) {
                return true;
            }
            else {
                ...
                for(int session = 1; session < 10; session++) {
                    if(SymbolInfoSessionQuote(symbol, day_of_week, session, from, to)) {
                        if(now >= from && now <= to) {
                            return true;
                        }
                    }
