        ...
        if(JsonReceived["status"].ToStr() == "success") {
            ...
            for(int i = 0; i < signalsCount; i++) {
                ...
                if(AutoTrade) {
                    ...
                    // Alert the user
                    string alertMessage = StringFormat("[GENESIS] %s Signal [%s] – (Entry %.4f  SL %.4f  TP %.4f)", 
...
//+------------------------------------------------------------------+
//| Count live trades (open positions + pending orders) per symbol   |
//+------------------------------------------------------------------+
int CountActiveTrades(const string sym)
{
   int cnt = 0;
   ...
   {
      ulong posTk = PositionGetTicket(i);
      if(!PositionSelectByTicket(posTk))          // safety
         continue;
   ...
   {
      ulong ordTk = OrderGetTicket(i);            // index → ticket
      if(ordTk == 0)                              // 0 ⇒ invalid
         continue;
      if(!OrderSelect(ordTk))                     // select by ticket
         continue;
...
void ProcessTradeSignal(int signalId, string symbol, string direction, double entry, double stopLoss, double takeProfit, double lotSize, bool force_execution=false)
{
    ...
    if(!IsMarketOpenForSymbol(symbol, force_execution)) {
       ...
       // ------------------------------------------------------------------
   //  Position cap – no more than 3 live trades per symbol
   // ------------------------------------------------------------------
   const int MAX_TRADES_PER_SYMBOL = 3;          // tweak if ever needed
   if(CountActiveTrades(symbol) >= MAX_TRADES_PER_SYMBOL)
   {
      DBG("Max trades (", MAX_TRADES_PER_SYMBOL, ") already open for ", symbol,
          " – skipping signal #", signalId);
   
      if(SendReports)
         SendTradeReport(signalId, symbol, direction, 0, lotSize, 0,
                         stopLoss, takeProfit, "ignored", "trade-cap reached");
      return;
   }
...
void DrawSignalArrow(int signalId, string symbol, string direction, double entryPrice)
{
    ...
    // Offset: ~USD 2 for gold (point=0.01 on XAUUSD → 200*point = $2)
...
bool SendPostRequest(string url,            // endpoint
                     ...
                     string &responseText)  // ← filled on success
{
   const int TIMEOUT_MS  = API_TIMEOUT;     // e.g. 10000
   const int MAX_RETRIES = 3;
   int       retryDelay  = 1000;            // 1 s → 2 s → 4 s
   ...
   //—— build payload as RAW BUFFER (char[]) ——————————————
   char bodyBuf[];
   ...
   for(int attempt = 1; attempt <= MAX_RETRIES; attempt++)
   {
      ...
      int http = WebRequest("POST",          // method
                      url,             // endpoint
                      reqHeaders,      // goes into the “cookie” slot
      ...
      //—— server rejected or fatal transport ————————
      string servReply = CharArrayToString(replyBuf, 0,
                                           ArraySize(replyBuf));
      DBG("WebRequest failed – HTTP ", http,
...
//───────────────────────────────────────────────────────────────────────
//  Function : SendTradeUpdates  (batched, ≤20 trades per WebRequest)
//───────────────────────────────────────────────────────────────────────
void SendTradeUpdates()
{
   DBG("======= STARTING TRADE UPDATES =======");

   //──────────────── 1. build the full 'trades' object ───────────────
   ...
   for(int offset = 0; offset < keyCount; offset += MAX_TRADES_PER_UPDATE)
   {
      ...
      if(success)
      {
         DBG(StringFormat("✓ Sent trade chunk %d–%d  (%s)",
      ...
      {
         DBG(StringFormat("Chunk %d–%d FAILED – retry next cycle",
                          offset+1, upto));
   ...
   if(!hasData)
      DBG("Snapshot contained no open or closed trades – server will mark account flat");
...
void FireSignalAlert(string direction, string symbol, double price)
{
    ...
    int symbolDigits = GetSymbolDigits(symbol);
    string msg = StringFormat("[GENESIS] %s Signal [%s] – (%s)", direction, symbol, DoubleToString(price, symbolDigits));
    Alert(msg);
}