...
if __name__ == "__main__":
    with app.app_context():
        # Create the signal within the app context
        signal = create_high_confidence_signal()
        
        # Fetch the signal details directly from the database to ensure we have the ID
        created_signal = db.session.get(Signal, signal.id)
        
        # Create a dictionary with all the signal details
        signal_info = {
            "id": created_signal.id,
            "symbol": created_signal.symbol,
            "action": created_signal.action.value if hasattr(created_signal.action, 'value') else str(created_signal.action),
            "entry": created_signal.entry,
            "sl": created_signal.sl,
            "tp": created_signal.tp,
            "confidence": created_signal.confidence,
            "mt5_symbol": created_signal.symbol.replace('_', '')
        }
        
        # Print the signal information
        print(f"Created high-confidence signal:")
        print(f"  ID: {signal_info['id']}")
        print(f"  Symbol: {signal_info['symbol']}")
        print(f"  Action: {signal_info['action']}")
        print(f"  Entry: {signal_info['entry']}")
        print(f"  Stop Loss: {signal_info['sl']}")
        print(f"  Take Profit: {signal_info['tp']}")
        print(f"  Confidence: {signal_info['confidence']}")
        print(f"\nTest with:")
        print(f"curl -X POST -H 'Content-Type: application/json' -d '{{\"account_id\":\"163499\",\"last_signal_id\":0,\"symbols\":[\"{signal_info['mt5_symbol']}\"]}}' http://localhost:5000/mt5/get_signals")
